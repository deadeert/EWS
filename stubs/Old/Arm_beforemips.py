...
def BREAK(it):
  return int.to_bytes(it<<6|BREAK_INSN_MIPS,4,'little')
...
class memset_arm():
    ...
    def __init__(self,itnum,arch):
        ...
        if arch == 'arm':
          ...
          self.insn_it = int.to_bytes(mipsbreak(self.idx_it,BREAK_INSN_MIPS),4,'little')
...
class strtok_arm():
    ...
    def do_it(self,helper):
        ...
        if str_addr != 0:
            ...
            if str.find(delem) == -1:
                helper.set_return(str_addr) # useless, just for reader comprehension, if token is not find, str ptr is returned.
                return
            else:
